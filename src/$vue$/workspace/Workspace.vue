...
const dropHandle = (ev: DragEvent) => {
    ...
    if (file) {
        dropItemEv(file, "/user/temp/")?.then((path: string) => {
            if (path) {
                fileActions?.(path);
            }
        });
...
onMounted(() => {
    ...
    const listItems = props.lists?.[0] || [];
    if (labelsRef.value) {
        inflectInGrid(labelsRef.value, props.items, listItems, createLabel);
    }
    if (shapesRef.value) {
        inflectInGrid(shapesRef.value, props.items, listItems, createShaped);
    }
});
</script>
...
<template>
    <ui-orientbox :orient="0" ref="topElement" data-alpha="0" data-chroma="0" data-scheme="base" class="u2-desktop-grid"
        ...
        <ui-gridbox class="u2-grid-page" style="background-color: transparent; inline-size: 100%; block-size: 100%;"
            ref="labelsRef"></ui-gridbox>
        <ui-gridbox class="u2-grid-page" style="background-color: transparent; inline-size: 100%; block-size: 100%;"
            ref="shapesRef"></ui-gridbox>
    </ui-orientbox>
</template>
