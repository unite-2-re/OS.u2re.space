...
export default (task: Task, )=>{
    ...
    <${"div.adl-main"} style="content-visibility:visible;" dataset=${{alpha: 0, chroma: 0, scheme: "solid"}}>
        <${"ui-scrollbox.adl-tab-box"} style="content-visibility:visible;" dataset=${{scheme: "solid", alpha: 1, highlight: 0.5, chroma: 0.01}}></ui-scrollbox>
        <${"ui-scrollbox.adl-content-box"} style="content-visibility:visible;" dataset=${{scheme: "solid", alpha: 1}}>
            <${"div.adl-content"} style="content-visibility:visible;" on:drop=${dropHandle} on:dragover=${dragOverHandle} iterate=${observableByMap(current)}>${(entry)=>{
